...
const createRecipe = async (req, res) => {
    const { name, image, cuisine, ingredients, method, time, category, servings, videoUrl } = req.body;
    let emptyFields = [];

    if (!name) { emptyFields.push('name'); }
    if (!image) { emptyFields.push('image'); }
    if (!cuisine) { emptyFields.push('cuisine'); }
    if (!Array.isArray(ingredients) || ingredients.length === 0) { emptyFields.push('ingredients'); }
    if (!method) { emptyFields.push('method'); }
    if (!videoUrl) { emptyFields.push('videoUrl'); } // Add validation for videoUrl if needed
